#!/usr/bin/env python3
"""
ReTrack Unlearning for Full Fine-tuning Models (FFSD Version)
...
def remove_style_from_prompt(prompt: str) -> str:
    ...
    FFSD prompts follow pattern: "{Subject Template}, artistic style featuring {descriptors}"
    This function removes ", artistic style featuring {descriptors}" part.
    
    Args:
        prompt: Original prompt with style
    
    Returns:
        Prompt with style removed (Subject Template only)
    """
    # Split by the specific separator used in FFSD prompts
    separator = ", artistic style featuring "
    if separator in prompt:
        return prompt.split(separator)[0].strip()
    
    # Fallback for original UC prompts if mixed (optional but good for safety)
...
def extract_descriptors_from_prompt(prompt: str) -> list:
    ...
    FFSD prompts: "{Subject Template}, artistic style featuring {descriptor1}, {descriptor2}, ..."
    
    Args:
        prompt: Original FFSD prompt
    
    Returns:
        List of descriptors (e.g., ["vibrant", "impasto", "warm tones"])
    """
    separator = ", artistic style featuring "
    if separator in prompt:
    ...
    # Fallback: empty list if pattern not found
    return []
...
def extract_object_from_prompt(prompt: str) -> str:
    ...
    FFSD prompts: "A depiction of {Object}, artistic style featuring ..."
    
    Args:
        prompt: Original FFSD prompt
    
    Returns:
        Object string (e.g., "Architectures", "dog")
    """
    # Extract everything before the separator
    subject_template = remove_style_from_prompt(prompt)
    
    # Pattern: "A depiction of {Object}" or similar
    # Try various common patterns
    patterns = [
        r'A depiction of (.+)',
        r'a photo of (.+)',
        r'an image of (.+)',
        r'(.+)'  # Fallback: use entire subject template
    ]
...
def encode_descriptors_with_clip(
    ...
    Returns:
        L2-normalized embedding vector (shape: (768,))
    """
...
def compute_style_distribution(
    ...
    """
    Compute style selection distribution π_s(s | c_f) with temperature and uniform mixing.
    
    Args:
        forget_embedding: CLIP embedding of forget descriptors (768,)
        style_prototypes: Pre-computed style prototypes (60, 768)
        style_names: List of style names in order
        forget_style_name: Name of forget style (to exclude)
        beta: Temperature parameter (higher = sharper distribution)
        eta_uniform: Uniform mixing weight (0.3 = 30% uniform, 70% softmax)
        sampling_mode: Style sampling mode ("weighted" or "uniform")
        device: Device to run on
    
    Returns:
        Probability distribution over styles (shape: (60,))
    """
    # Move to device
    forget_embedding = forget_embedding.to(device)
    style_prototypes = style_prototypes.to(device)
    
    # Uniform distribution over non-forget styles
    ...
    logits = beta * torch.matmul(forget_embedding, style_prototypes.t())  # (60,)
    logits[forget_idx] = -float('inf')

    # Softmax to get distribution
    pi_softmax = torch.softmax(logits, dim=0)

    # Mix: (1 - eta) * softmax + eta * uniform
...
def synthesize_retain_prompt(
    ...
    Template: "{object}, artistic style featuring {descriptor1}, {descriptor2}, {descriptor3}"
    
    Args:
        object_name: Object to keep (e.g., "Architectures", "dog")
        style_descriptors: List of descriptors for sampled style
        num_anchor_descriptors: Number of descriptors to sample
    
    Returns:
        Synthesized prompt string
    """
    # Format: "A depiction of {Object}, artistic style featuring {descriptors}"
    subject_template = f"A depiction of {object_name}"
    
    # Randomly shuffle and select up to 3 descriptors (for variety)
    import random
...
class UnlearnDataset(Dataset):
        ...
        with open(prompts_file, 'r') as f:
            for line in f:
                ...
                if len(parts) >= 4:
                    ...
                    image_path = self._resolve_image_path(data_root, rel_path)
    ...
    @staticmethod
    def _resolve_image_path(data_root: Path, rel_path: str) -> Path:
        """Resolve prompt paths for either data/ or data/UnlearnCanvas roots."""
        rel = Path(rel_path)
        direct = data_root / rel
        if direct.exists():
            return direct
        parts = rel.parts
        if parts and parts[0] == "UnlearnCanvas":
            without_prefix = data_root / Path(*parts[1:])
...
def generate_sample_images(
    ...
    for style in test_styles:
        for obj in test_objects:
            # Use simple prompt for generation monitoring
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["matched_retain", "style_removed", "weighted_style_select"],
        help="Anchor strategy: matched_retain (same object/ID, different style), style_removed (remove style from prompt), or weighted_style_select (CLIP-based style selection)"
    )
    parser.add_argument(
        "--beta",
        type=float,
        default=2.0,
        help="Temperature parameter for weighted_style_select strategy (higher = sharper distribution)"
    )
    parser.add_argument(
        "--eta_uniform",
        type=float,
        default=0.3,
        help="Uniform mixing weight for weighted_style_select strategy (0.3 = 30%% uniform, 70%% softmax)"
    )
    parser.add_argument(
        "--style_sampling_mode",
        type=str,
        ...
        help="How to sample retain styles for weighted_style_select"
    )
    parser.add_argument(
        "--num_anchor_descriptors",
        type=int,
        default=3,
        help="Number of descriptors used when synthesizing the anchor prompt"
    )
    parser.add_argument(
        "--clip_prototypes_file",
        type=str,
        default="cache/clip_style_prototypes_very_relaxed.pt",
        help="Path to pre-computed CLIP style prototypes (for weighted_style_select strategy)"
    )
    parser.add_argument(
        "--descriptors_file",
        type=str,
        default="data/ffsd_very_relaxed_descriptors.json",
        help="Path to style descriptors JSON file (for weighted_style_select strategy)"
    )
...
def main():
    ...
    print(f"ReTrack Unlearning (Full Fine-tuning) - FFSD Version")
    print(f"=" * 80)
    ...
    if args.anchor_strategy == "weighted_style_select":
        print(f"Beta (temperature): {args.beta}")
        print(f"Eta uniform: {args.eta_uniform}")
    print(f"=" * 80 + "\n")
    ...
    if args.anchor_strategy == "weighted_style_select":
        ...
        # Load pre-computed style prototypes
        ...
        style_prototypes = prototype_data['prototypes']  # (60, 768)
    ...
    while global_step < args.num_steps:
        for batch in dataloader:
            ...
            if args.anchor_strategy == "matched_retain":
                ...
                # Strategy 3: weighted_style_select - CLIP-based style selection
                anchor_indices = None
                synthesized_prompts = []
                
                for prompt_f in prompts_forget:
                    # Extract descriptors and object from forget prompt
                    descriptors_f = extract_descriptors_from_prompt(prompt_f)
                    object_f = extract_object_from_prompt(prompt_f)
                    
                    # Encode forget descriptors with CLIP
                    with torch.no_grad():
                    ...
                    # Compute style selection distribution
                    ...
                    # Synthesize retain prompt with same object, different style
                ...
                # Encode synthesized prompts with SD text encoder
                with torch.no_grad():
