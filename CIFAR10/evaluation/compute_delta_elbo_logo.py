#!/usr/bin/env python3
"""
Compute per-class ΔELBO using leave-one-out models for CIFAR-10/100 samples.
This script is memory-efficient by loading one model at a time.
"""
import argparse
import csv
import os
...
def get_args() -> argparse.Namespace:
    ...
    p.add_argument("--ckpt_all", required=True, help="Checkpoint for model trained on all classes")
    p.add_argument("--ckpt_base", required=True, help="Base directory containing exclude_X models")
    p.add_argument("--ckpt_epoch", type=str, default="epoch_1200", help="Epoch directory name of the checkpoint.")
    p.add_argument("--out_csv", default="delta_elbo_loo.csv", help="Output CSV filename")
    ...
    p.add_argument("--base_elbo_cache", default=None, help="Path to pre-computed base ELBO cache (.npz)")
    return p.parse_args()
...
def main() -> None:
    ...
    # --- Load base model (trained on all classes) once ---
    print("Loading base model (all classes)...")
    all_model = UNet2DModel.from_pretrained(args.ckpt_all).to(device).eval()
    sched = DDPMScheduler.from_pretrained(args.ckpt_all)
    sched.set_timesteps(args.timesteps)
    diffusion_helpers = DiffusionHelpers(sched)
    
    # --- Load base ELBO cache if provided ---
    base_elbo_cache = None
    if args.base_elbo_cache:
    ...
    for s in tqdm(range(0, len(paths), args.batch), desc="Processing Batches"):
        ...
        # 2. Pre-calculate KL divergence for the 'all_model' for all timesteps
        kl_all_per_t = {}
        ...
        if base_elbo_cache is not None:
            ...
            for t_idx, t_val in enumerate(timesteps_to_process):
                ...
                kl_all_per_t[t_val] = torch.stack([
            ...
            for t_val in timesteps_to_process:
                ...
                raw_all = all_model(x_t, t_batch).sample
                eps_all = extract_eps(raw_all, all_model)
                x0_hat_all = diffusion_helpers._predict_xstart_from_eps(x_t, t_batch, eps_all)
                mu_p_all, _, _ = diffusion_helpers.q_posterior_mean_variance(x_start=x0_hat_all, x_t=x_t, t=t_batch)
                
                kl_all = normal_kl(mu_q.double(), log_var_q.double(), mu_p_all.double(), log_var_q.double())
                kl_all_per_t[t_val] = kl_all.to(device)

        # 3. Loop through each class, load the corresponding model, and compute delta
        for class_idx in tqdm(range(num_classes), desc="Processing Classes", leave=False):
            exclude_path = os.path.join(args.ckpt_base, f"exclude_{class_idx}", args.ckpt_epoch)
            if not os.path.exists(exclude_path):
                print(f"Warning: Skipping exclude_{class_idx}, path not found: {exclude_path}")
                continue

            # Load one exclude_model at a time
            exclude_model = UNet2DModel.from_pretrained(exclude_path).to(device).eval()
            ...
            for t_val in timesteps_to_process:
                ...
                raw_exc = exclude_model(x_t, t_batch).sample
                eps_exc = extract_eps(raw_exc, exclude_model)
                x0_hat_ex = diffusion_helpers._predict_xstart_from_eps(x_t, t_batch, eps_exc)
                mu_p_ex, _, _ = diffusion_helpers.q_posterior_mean_variance(x_start=x0_hat_ex, x_t=x_t, t=t_batch)
                
                kl_ex = normal_kl(mu_q.double(), log_var_q.double(), mu_p_ex.double(), log_var_q.double())
                
                # Retrieve pre-calculated kl_all and compute delta for this timestep
                delta_for_this_class += (kl_ex.to(device) - kl_all_per_t[t_val]).to(torch.float32)
